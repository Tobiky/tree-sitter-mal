...
class malAnalyzer():
    ...
    def _analyse_extends(self) -> None:
        ...
        for asset in self._assets:
            ...
            if(asset_node.child_by_field_name('extends')):
                ...
                extend_asset_name = asset_node.child_by_field_name('extends').next_sibling.text.decode()
        ...
        for parent in self._assets:
            ...
            if(parent_node.children[0].text.decode()=='abstract'):
                ...
                for extendee in self._assets:
                    ...
                    extendee_node_extender = extendee_node.child_by_field_name('extends')
                    if(extendee_node_extender and \
                           extendee_node_extender.next_sibling.text.decode() == parent_node_name):
                        found = True
                        break
                if not found:
                    self._warn = True
                    logging.warning(f'Asset \'{parent_node_name}\' is abstract but never extended to')
    ...
    def _analyse_parents(self) -> None:
        ...
        for asset in self._assets:
            parents: list[str] = []
            parent_node: malParser.AssetContext  = self._assets[asset]['node']
                    ...
                    if (attack_step['requires']):
                        ...
                        for expr in attack_step['requires']['stepExpressions']:
                            if not self._check_to_asset(asset, expr):
                                error_msg = f"Line {self._steps[asset][attack_step['name']]['node'].start_point.row}: " + \
        ...
        if asset not in self._associations or field not in self._associations[asset]:
            ...
            if not step_node:
                if not asset in self._associations.keys():
                    self._associations[asset] = {field: {k: association[k] for k in ["association", "node"]}}
                else:
                    self._associations[asset][field] = {k: association[k] for k in ["association", "node"]}
        ...
        for asset in self._assets.keys():
            ...
            for parent in parents:
                ...
                for var in self._vars[asset].keys():
                    if parent in self._vars.keys() and var in self._vars[parent].keys() and self._vars[asset][var]['node']!=self._vars[parent][var]['node']:
    ...
    def _get_LCA(self, lhs_target, rhs_target):
        ...
        if (self._is_child(lhs_target, rhs_target)):
            ...
            if (not lhs_parent_node or not rhs_parent_node):
                return None
            return self._get_LCA(lhs_parent_node.child_by_field_name('id').text.decode(), rhs_parent_node.child_by_field_name('id').text.decode())
    ...
    def _is_child(self, parent_name, child_name):
        ...
        if (valid_asset := self._get_asset_name(child_name)):
            ...
            if (parent_node := self._get_assets_extendee(asset_node)):
                child_parent_name = self._get_asset_name(parent_node.child_by_field_name('id').text.decode())
            ...
            for attackStep in self._steps[parent].keys():
                ...
                if attackStep not in current_steps:
                    ...
                    if not prevDef_parent:
                        ...
                        attackStep_node_reaches = attackStep_node.child_by_field_name('reaches')
                        if (not attackStep_node_reaches or attackStep_node_reaches.child_by_field_name('operator').text.decode()=='->'):
                            # Valid step
                        ...
                        attackStep_node = self._steps[parent][attackStep]['node']
                        parent_attackStep_node = self._steps[prevDef_parent][attackStep]['node']
                        if attackStep_node.child_by_field_name('step_type').text==parent_attackStep_node.child_by_field_name('step_type').text:
                            # Valid step
                            current_steps.append(attackStep)
                        else:
    ...
    def check_asset_declaration(self, node: Node, asset: dict) -> None:
        ...
        if (not asset_name):
            logging.error(f"Asset was defined without a name at line {node.start_point.row}")
            self._error = True
            return
        ...
        if asset_name in self._assets.keys(): 
            ...
            self._assets[asset_name] = {'node': node, 'obj': asset, 'parent': {'name': category_name,'node': node.parent}}
    ...
    def check_meta(self, node: Node, data: Tuple[str, str]) -> None:
        ...
        elif node.parent in self._metas.keys() and meta_name not in self._metas[node.parent]:
            self._metas[node.parent][meta_name] = node
    ...
    def check_category_declaration(self, node: Node, data: Tuple[str, Tuple[List, Any]]) -> None:
        '''
        ...
        self._category[category['name']] = {'node': node, 'obj': {'category': category, 'assets': assets}}
    ...
    def check_association(self, node: Node, association: dict):
        self._all_associations.append({'name': association['name'], 'association': association,'node':node})
